...
class AnomalyTreaterSts(object):
    def __init__(self, loadfile):
        ...
        vno = vn.VectorNorm(loadfile, confd_level=1.5, norm=2)
        vno.find_anomalies()
        vn_an = vno.get_anomalies()
        pdfo = pdf.PDF(loadfile, confidence_level=2.0, cutoff=15)
    ...
    def replace_anomalies(self, use_OR=False):
        ...
        data = pd.read_csv(self.loadfile)
    ...
    def save_rectified_data(self):
        newfile = self.loadfile[:-4] + '_A.csv'
...
if __name__ == "__main__":
    ats = AnomalyTreaterSts('TPC_Load.csv')
